...
local t,p = ffi_new("struct json_token"), ffi_new("struct json_pair")
...
val[1] = function(v)
    ...
    for n = 1, l do
        ...
        o[val[4](p.name)] = val[tonumber(lib.json_type(p.value))](p.value)
    end
    return o
end
val[2] = function(v)
...
val[4] = function(v) lib.json_deq(v); return ffi_str(buf, lib.json_cpy(buf, tonumber(v.len) + 1, v)) end
